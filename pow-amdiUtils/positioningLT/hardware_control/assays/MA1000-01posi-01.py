...
SHORT = 0               # testing, limited to all images, one short BPSSS spin and
                        # the LV1(a/b) cut. This is for debug purposes only.
...
import logging
import inspect
from settings import LOGGING_FILE, SI_COUNT, CAL_DIR, POSI_FILE
...
def get_sample_image(self,
    ...
    for tried in range(tries):
        ...
        if res == 0:
            ...
            tried += 1
            msg = 'Erk, didn\'t get a picture, retrying [%d/%d]' % (tried+1, tries)
            logging.info(msg)
            print(msg)
...
def run_assay(self, filename, run):
    '''
    ...
    msg = 'Using %s model %s for the assay test' % (cartridge, model)
    logging.info(msg)
    print(msg)
    ...
    SWING2       = round(2.0 * float(TICKS))  # LV5/7 cuts are smaller angles
    ...
    if EXPERIMENTAL:
        ...
        CAL_location = ((bump
                        + round(CAL_CAMERA - ((68 + 18 - 1))
                        * float(TICKS)))
                        + SI_COUNT) % SI_COUNT  # 82 deg (corrected to be +ve)
        ...
        gray_cropped = gray[int(height*0.3):height, 0:width].copy()
        if DEBUG_IMGS:
        ...
        # we don't need it to be full size, a quarter size would do fine for testing
        height, width, depth = img.shape
        cv2.resize(img, RESIZE)

        # threshold
        thresh = cv2.adaptiveThreshold(gray_cropped, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 13, 3)
        thresh = 255 - thresh
        if DEBUG_IMGS:
        ...
        if CHECKING:
            ...
            self.motor.position_abs(round((CAL_CAMERA - ((67 + 18) * float(TICKS))) + SI_COUNT) % SI_COUNT)
    ...
    print('LV1-7:     %04d %04d %04d %04d %04d %04d %04d'
          % (LV1_location,
             LV2_location,
             LV3_location,
             LV4_location,
    ...
    print('LV1-7+off: %04d %04d %04d %04d %04d %04d %04d'
          % (LV1_SAMPLE[0],
             LV2_WASH1[0],
             LV3_ANTIBODY[0],
             LV4_WASH2[0],
    ...
    # logging.info('POSI: LV1-7+off: %04d %04d %04d %04d %04d %04d %04d'
    #       % (LV1_SAMPLE[0],
    #          LV2_WASH1[0],
    #          LV3_ANTIBODY[0],
    #          LV4_WASH2[0],
    ...
    if VERBOSE != 0:
        ...
        cartridge = assay_id.split('-')[0]
        if cartridge == 'MA1010':
            logging.info('We are using a %s cartridge with ramjets!', cartridge)
        else:
    ...
    if NOLASER == 0:
        for i in range(1,10):
            ...
            # if VERBOSE != 0:
            #     if (i % 2) == 1:
            #         msg2 = 'Sleeping %d seconds to let laser cool' % EXTRA_COOL
            #         msg = 'not happy:/'
            #         logging.info(msg)
            #         print(msg)
                    

            logging.info('positioning rig')
            #position = round(LV1_SAMPLE[i % len(LV1_SAMPLE)])
            position = round(LV1_SAMPLE[0])
            msg = 'Moving to LV1# @ %d ticks (%d°)' \
                   %  (position,
    ...
    if not SHORT:
        ...
        if NOLASER == 0:
            for i in range(len(LV2_WASH1)):
                ...
                # logger_P.info('Current position b4 LV2 C: %d' % posi)
                if self.laser_sweep(start_tick=position,
                                    width=SWING,
                                    delay=DELAY,
                                    steps=STEPS_INNER,
        ...
        if NOLASER == 0:
            for i in range(len(LV3_ANTIBODY)):
                ...
                # posi = self.motor.get_actual_encoder_count()
                # logger_P.info('Current position b4 LV3 C: %d' % posi)
                if self.laser_sweep(start_tick=position,
                                    width=SWING,
                                    delay=DELAY,
                                    steps=STEPS_INNER,
        ...
        if NOLASER == 0:
            for i in range(len(LV4_WASH2)):
                ...
                # posi = self.motor.get_actual_encoder_count()
                # logger_P.info('Current position b4 LV4 C: %d' % posi)
                if self.laser_sweep(start_tick=position,
                                    width=SWING,
                                    delay=DELAY,
                                    steps=STEPS_INNER,
        ...
        if NOLASER == 0:
            for i in range(len(LV5_DRY)):
                ...
                # posi = self.motor.get_actual_encoder_count()
                # logger_P.info('Current position b4 LV5 C: %d' % posi)
                if self.laser2_sweep(start_tick=position,
                                     ...
                                     lv=5,
                                     side=i,
                                     rn=run) == -1:  # WET VALVE! use more power and steps
                    return -2
        ...
        if NOLASER == 0:
            for i in range(len(LV6_SPIN)):
                ...
                # posi = self.motor.get_actual_encoder_count()
                # logger_P.info('Current position b4 LV6 C: %d' % posi)
                if self.laser_sweep(start_tick=position,
                                    width=SWING,
                                    delay=DELAY,
                                    steps=STEPS_INNER,
                                    laser_intensity=INTENSITY_DRY,
                                    lv=6,
                                    side=i,
                                    rn=run) == -1:
                    return -2
        ...
        # ------------------------------------------------------------------
        # Open laser valve 7 - Open the outer bay door HAL... (we want
        #                      to open the ramjets)
        
        DEPRICATED!
        if NOLASER == 0:
            for i in range(len(LV7_RAMJET)):
                # ~ if VERBOSE != 0:
                position = round(LV7_RAMJET[i % len(LV7_RAMJET)])
                msg = 'Moving to LV7#%d @ %d ticks (%d°)' \
                       % (i % len(LV7_RAMJET),
                          position,
                          position / (SI_COUNT/360))
                logging.info(msg)
        ...
        # valves 5,6 & 7 are now open so we can dump everything to
        # the waste chamber
        # Dry the wells
