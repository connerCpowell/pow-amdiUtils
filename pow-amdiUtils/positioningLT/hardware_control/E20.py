...
class E20:
    ...
    def run_assay(self, filename=None, run='1', version='01'):
        if self.cover_open():
        ...
        try:
            ...
            assay_return = assay.run_assay(self, filename, run)
            ...
            if default is not None:
                ...
                default_return = default.run_assay(self, filename, run)
            else:
    ...
    def psd(self, lv, side, rn):
        ...
        if GPIO.input(LV1_PS_PIN) == LV1_PS_ON:
            ...
            logging.info('POSI: lv1 was detected @ positon: $%s $P $%d $YES $%d $%d' % (lv, count, side, rn))
            #i = False
            ...
            count = self.motor.get_actual_encoder_count()
            print('position=',count)
            logging.info('POSI: lv1 NOT detected @ positon: $%s $P $%d $NO $%d $%d' % (lv, count, side, rn))

    def psd2(self, lv, side, rn):
        
        if GPIO.input(LV2_PS_PIN) == LV2_PS_ON:
            print('laser2 is detected')
            count = self.motor.get_actual_encoder_count()
            print('position=',count)
            logging.info('POSI: lv2 was detected @ positon: $%s $P $%d $YES $%d $%d' % (lv, count, side, rn))
            #i = False
            ...
            count = self.motor.get_actual_encoder_count()
            print('position=',count)
            logging.info('POSI: lv2 NOT detected @ positon: $%s $P $%d $NO $%d $%d' % (lv, count, side, rn))

    def FBpsd(self, lv, side, rn):
    ...
    def laser_sweep(self,
        ...
        if pattern == 'C':
            ...
            logging.info('POSI: position b4 lv#$%s $B $%d $N/A $%d $%d' % (lv, posi2, side, rn))
            ...
            logging.info('POSI: position af lv#$%s $A $%d $N/A $%d $%d' % (lv, posi, side, rn))
        ...
        # --------------------------------------------------------------
            ...
            logging.info('lasers on')
            step = 5
            increment = smallest
            logging.info('inc set')
            while increment <= (start_tick + (width/2)):  # DONT compare with modulo (largest)!
                increment += step
                logging.info('inside while')
                
                self.motor.position_rel(step)
                logging.info('steps')
                if self.FBpsd(lv, side, rn) == True:
                    logging.info('checked')
                    sleep(4)
                    break

            logging.info('go to middle')
            self.laser1.off()
            self.motor.position_abs(middle)
            while not self.motor.info(check_bit=MOVE_COMPLETED):
                pass
