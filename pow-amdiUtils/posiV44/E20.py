...
from settings import LOGGING_FILE, LOGGING_LEVEL, SI_COUNT, CALIBRATION_FILE
...
from settings import MOVE_COMPLETED, HALTED, SETTLE
from settings import VELOCITY_MODE_ACTIVE, POSITION_MODE_ACTIVE, TORQUE_MODE_ACTIVE
...
class E20:
    ...
    def cover_open(self):
        ...
        if GPIO.input(COVER_CHECK_PIN) == COVER_OPEN:
            ...
            if self.end_of_test_operations == False:
                logging.critical('Cover opened, laser power disabled, motor stopped')
                logging.critical('Unfinished test! ABORT! (reject the disk and start over)')
    ...
    def run_assay(self, filename=None, version='01'):
        if self.cover_open():
            ...
            logging.error('Test stopped, lid/cover open')
            return LID_OPEN
        ...
        try:
            ...
            if default is not None:
                ...
                logging.warn('Requested %s recipe not found. Using default [%s]' % (filename[0:13], DEFAULT_RECIPE))
    ...
    def psd(self, lv, side, rn):
        
        if GPIO.input(LV1_PS_PIN) == LV1_PS_ON:
            #logging.info('psder')
            print('laser1 is detected')
            count = self.motor.get_actual_encoder_count()
            print('position=',count)
            logging.info('POSI: lv1 was detected @ positon: $%s $P $%s $YES $%s $%s' % (lv, count, side, rn))
            #i = False
            ...
            count = self.motor.get_actual_encoder_count()
            print('position=',count)
            logging.info('POSI: lv1 NOT detected @ positon: $%s $P $%s $NO $%s $%s' % (lv, count, side, rn))
    ...
    def laser_sweep(self,
        ...
        if pattern == 'C':
            ...
            # Are we there yet?
            while not self.motor.flag_info(check_bit=MOVE_COMPLETED):
                pass

            # Did we at least stop??
            while not self.motor.flag_info(check_bit=HALTED):
                pass
            ...
            # logging.info('posi error')
            logging.info('POSI: position b4 lv#$%s $B $%s $N/A $%s $%s' % (lv, posi2, side, rn))
            ...
            logging.info('POSI: position af lv#$%s $A $%s $N/A $%s $%s' % (lv, posi, side, rn))
