...
from settings import HOME_ACCEL, HOME_SPEED, DEBUGGING, LOGGING_LEVEL
...
def run_assay(self, filename):
    ...
    # Calibration
    TOLERANCE = 2 # Maximum acceptable variance in deg from optical checking
                  # before using baked defaults from calibration file
    ...
    if not SHORT:
        BPS_TIME = 6    # length of time to spin to get seperation of plasma (PRP)
    else:
        BPS_TIME = 6
    ...
    def spin_oscilate(loops=REPEATS):
        ...
        '''
        self.motor.change_pid('high')  # prepare for high speed manovers
    ...
    if NOLASER == 0:
        for i in range(1,1000):
            ...
            logging.info('goin home #%d' % i)
            self.motor.position_abs(0)
